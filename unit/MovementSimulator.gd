...
func RegisterAgent(agent: MovementAgent) -> bool:
	...
	if (
		...
		and not _navigationService.CanPlaceStatic(agent.position, agent.halfSize)
	):
...
func _wouldOverlapAgent(position: Vector2, halfSize: int, ignoredUnitId: int) -> bool:
	for otherAgent: MovementAgent in _agents:
		...
		var combinedHalfSize: float = maxf(
			0.0,
			float(halfSize + otherAgent.halfSize) - COLLISION_MARGIN,
		)

		if (
			absf(position.x - otherAgent.position.x) < combinedHalfSize
			and absf(position.y - otherAgent.position.y) < combinedHalfSize
		):
			return true
...
func _accumulatePairAvoidance(firstAgentIndex: int, secondAgentIndex: int) -> void:
	var firstMovementDelta: Vector2 = _workingPositions[firstAgentIndex] - _startPositions[
		firstAgentIndex
	]
	var secondMovementDelta: Vector2 = _workingPositions[secondAgentIndex] - _startPositions[
		secondAgentIndex
	]
	...
	if firstMoves and secondMoves and _moveInSameDirection(firstMovementDelta, secondMovementDelta):
		var travelDirection: Vector2 = (
			firstMovementDelta.normalized() + secondMovementDelta.normalized()
		).normalized()
		...
		if firstProgress < secondProgress - Math.EPSILON:
			var firstSide: float = _getSideAwayFromBlocker(
				firstAgentIndex,
				secondAgentIndex,
				firstMovementDelta,
			)
			...
			_addAvoidance(
				firstAgentIndex,
				_getSideDirection(firstMovementDelta, firstSide),
				FOLLOW_SPEED_SCALE,
			)
			return

		if secondProgress < firstProgress - Math.EPSILON:
			var secondSide: float = _getSideAwayFromBlocker(
				secondAgentIndex,
				firstAgentIndex,
				secondMovementDelta,
			)
			...
			_addAvoidance(
				secondAgentIndex,
				_getSideDirection(secondMovementDelta, secondSide),
				FOLLOW_SPEED_SCALE,
			)
			return

	if firstMoves and secondMoves:
		var pairSide: float = _getPairRotationSide(
			firstAgentIndex,
			secondAgentIndex,
			firstMovementDelta,
			secondMovementDelta,
...
func _markArrivalBlocking(
	...
	if (firstAgent.moveCommandId < 0 or firstAgent.moveCommandId != secondAgent.moveCommandId):
		return
	...
	var secondInside: bool = secondDistance <= secondAgent.arrivalRadius + float(
		secondAgent.halfSize
	)
...
func _getPairRotationSide(
	firstAgentIndex: int,
	secondAgentIndex: int,
	firstDesiredDelta: Vector2,
	secondDesiredDelta: Vector2,
) -> float:
	...
	var positiveCollides: bool = _movementsIntersect(
		_startPositions[firstAgentIndex],
		firstPositiveEnd,
		_startPositions[secondAgentIndex],
		secondPositiveEnd,
		combinedHalfSize,
	)
	var negativeCollides: bool = _movementsIntersect(
		_startPositions[firstAgentIndex],
		firstNegativeEnd,
		_startPositions[secondAgentIndex],
		secondNegativeEnd,
...
func _getSideAwayFromBlocker(
	movingAgentIndex: int,
	blockerAgentIndex: int,
	movementDelta: Vector2,
) -> float:
	var sideDirection: Vector2 = _getSideDirection(movementDelta, 1.0)
	var separation: float = (
		_startPositions[movingAgentIndex] - _startPositions[blockerAgentIndex]
	).dot(sideDirection)

	if absf(separation) <= Math.EPSILON:
		separation = (
			_workingPositions[movingAgentIndex] - _workingPositions[blockerAgentIndex]
		).dot(sideDirection)
...
func _findAvoidancePosition(agentIndex: int, iteration: int) -> Vector2:
	...
	if iteration >= AVOIDANCE_ITERATION_COUNT - 1:
		...
		oppositeDirection = (
			desiredDirection + oppositeAvoidanceDirection * avoidanceStrength
		).normalized()

	var candidatePosition: Vector2 = _findFarthestStaticClearPosition(
		agentIndex,
		primaryDirection,
		stepDistance,
	)
	...
	candidatePosition = _findFarthestStaticClearPosition(
		agentIndex,
		oppositeDirection,
		stepDistance,
	)
...
func _findFarthestStaticClearPosition(
	agentIndex: int,
	direction: Vector2,
	stepDistance: float,
) -> Vector2:
...
func _applyHardCollisionFallback() -> bool:
	...
	for pairIndex: int in range(_collisionFirstAgentIndices.size()):
		var firstAgentIndex: int = _collisionFirstAgentIndices[pairIndex]
		var secondAgentIndex: int = _collisionSecondAgentIndices[pairIndex]
		var firstMoves: bool = not _workingPositions[firstAgentIndex].is_equal_approx(
			_startPositions[firstAgentIndex]
		)
		var secondMoves: bool = not _workingPositions[secondAgentIndex].is_equal_approx(
			_startPositions[secondAgentIndex]
		)
...
func _propagateHardStops(stoppedAgentIndices: Array[int]) -> void:
	...
	while queueIndex < stoppedAgentIndices.size():
		...
		var minCell: Vector2i = _getSweptMinCell(
			blockerPosition,
			blockerPosition,
			blockerAgent.halfSize,
		)
		var maxCell: Vector2i = _getSweptMaxCell(
			blockerPosition,
			blockerPosition,
			blockerAgent.halfSize,
		)
		...
		for cellX: int in range(minCell.x, maxCell.x + 1):
			for cellY: int in range(minCell.y, maxCell.y + 1):
				...
				for movingAgentIndex: int in cellAgentIndices:
					if (
						movingAgentIndex == blockerAgentIndex
						or _hardStopFlags[movingAgentIndex] != 0
					):
						continue
					...
					if _workingPositions[movingAgentIndex].is_equal_approx(
						_startPositions[movingAgentIndex]
					):
						continue

					var combinedHalfSize: int = _agents[movingAgentIndex].halfSize + blockerAgent.halfSize

					if _movementsIntersect(
						_startPositions[movingAgentIndex],
						_workingPositions[movingAgentIndex],
						blockerPosition,
						blockerPosition,
						combinedHalfSize,
					):
...
func _rebuildSweptSpatialIndex() -> void:
	...
	for agentIndex: int in range(_agents.size()):
		var agent: MovementAgent = _agents[agentIndex]
		var minCell: Vector2i = _getSweptMinCell(
			_startPositions[agentIndex],
			_workingPositions[agentIndex],
			agent.halfSize,
		)
		var maxCell: Vector2i = _getSweptMaxCell(
			_startPositions[agentIndex],
		...
		for cellX: int in range(minCell.x, maxCell.x + 1):
			for cellY: int in range(minCell.y, maxCell.y + 1):
				var cell: Vector2i = Vector2i(cellX, cellY)
				var cellAgentIndices: PackedInt32Array = _sweptAgentIndicesByCell.get(
					cell,
					PackedInt32Array(),
				)
...
func _collectCollisionPairs() -> int:
	...
	for firstAgentIndex: int in range(_agents.size()):
		...
		var minCell: Vector2i = _getSweptMinCell(
			_startPositions[firstAgentIndex],
			_workingPositions[firstAgentIndex],
			firstAgent.halfSize,
		)
		var maxCell: Vector2i = _getSweptMaxCell(
			_startPositions[firstAgentIndex],
		...
		for cellX: int in range(minCell.x, maxCell.x + 1):
			for cellY: int in range(minCell.y, maxCell.y + 1):
				...
				for secondAgentIndex: int in cellAgentIndices:
					...
					if not _movementsIntersect(
						_startPositions[firstAgentIndex],
						_workingPositions[firstAgentIndex],
						_startPositions[secondAgentIndex],
						_workingPositions[secondAgentIndex],
						combinedHalfSize,
					):
						continue
...
func _updateArrivalSettling() -> void:
	for agentIndex: int in range(_agents.size()):
		...
		var insideArrivalArea: bool = (
			agent.position.distance_to(agent.moveTarget) <= agent.arrivalRadius
			+ float(agent.halfSize)
		)
...
func _hasStaticLateralSpace(agent: MovementAgent) -> bool:
	...
	var checkDistance: float = maxf(float(agent.halfSize) * SETTLE_LATERAL_CHECK_SCALE, 1.0)
	...
	return (
		_navigationService.SegmentClear(agent.position, positiveTarget, agent.halfSize)
		or _navigationService.SegmentClear(agent.position, negativeTarget, agent.halfSize)
	)


func _movementsIntersect(
	firstStart: Vector2,
	firstEnd: Vector2,
	secondStart: Vector2,
	secondEnd: Vector2,
	combinedHalfSize: int,
) -> bool:
	...
	var startsOverlapping: bool = (
		absf(relativeStart.x) < collisionHalfSize and absf(relativeStart.y) < collisionHalfSize
	)
...
func _segmentIntersectsCenteredRect(
...
func _getSweptMinCell(startPosition: Vector2, endPosition: Vector2, halfSize: int) -> Vector2i:
	var minimumPosition: Vector2 = Vector2(
		minf(startPosition.x, endPosition.x) - float(halfSize),
		minf(startPosition.y, endPosition.y) - float(halfSize),
	)
	return _getCell(minimumPosition)


func _getSweptMaxCell(startPosition: Vector2, endPosition: Vector2, halfSize: int) -> Vector2i:
	var maximumPosition: Vector2 = Vector2(
		maxf(startPosition.x, endPosition.x) + float(halfSize),
		maxf(startPosition.y, endPosition.y) + float(halfSize),
	)
...
func _getCell(position: Vector2) -> Vector2i:
	return Vector2i(
		floori(position.x / StageSnapshot.SPATIAL_CELL_SIZE),
		floori(position.y / StageSnapshot.SPATIAL_CELL_SIZE),
	)
