...
@onready var _hud: TycoonHUD = $UI/UIRoot/HUD
...
func _ConnectControllerSignals() -> void:
	_tycoonController.SettlementChanged.connect(_RefreshHUDData)
	_tycoonController.DefenseRequested.connect(_OnDefenseRequested)
...
func _ConnectUISignals() -> void:
	_hud.EndTurnRequested.connect(_OnEndTurnRequested)
	_eventPopup.visibility_changed.connect(_RefreshEndTurnButton)
...
func _RefreshHUDData() -> void:
	_hud.Refresh(
		_tycoonController.GetCampaign(),
		_tycoonController.GetSettlement()
	)


func _RefreshEndTurnButton() -> void:
	_hud.SetEndTurnEnabled(
		_tycoonController.GetCampaign().currentPhase == CampaignState.Phase.TYCOON
		and not _tycoonController.GetEventState().HasActiveEvent()
		and not _eventPopup.visible
	)


func _OnDefenseRequested(_startData: DefenseStartData) -> void:
	_RefreshEndTurnButton()


func _OnOffenseRequested(_startData: OffenseStartData) -> void:
	# Offense 내부 턴 진행은 SettlementChanged를 생략하므로 여기서 데이터 동기화.
	_RefreshHUDData()
	_RefreshEndTurnButton()


func _OnOffenseFinished(_result: OffenseResult) -> void:
	# 복귀 데이터는 앞서 발생한 SettlementChanged에서 이미 갱신됨.
	_RefreshEndTurnButton()


func _OnEndTurnRequested() -> void:
	if (
		_eventPopup.visible
		or _tycoonController.GetEventState().HasActiveEvent()
		or _tycoonController.GetCampaign().currentPhase != CampaignState.Phase.TYCOON
	):
		return
	# 마지막 턴에서는 false를 반환하면서 DefenseRequested를 보내므로 오류로 취급하지 않음.
